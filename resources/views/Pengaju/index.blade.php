...
<div x-data="{ openDelete: false, deleteRoute: '', showImage: false, imgFull: '', PengajuNama: '' }" class="relative">
    ...
    <div class="p-4 md:p-6 min-h-screen bg-slate-50">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-xs md:text-sm text-slate-500 font-medium">Total Pengaju</p>
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
                <h1 class="text-xl md:text-2xl font-bold text-slate-900">Manajemen Data Pengaju</h1>
                <p class="text-xs md:text-sm text-slate-500">Daftar pengguna akses Pengajuistrator sistem.</p>
            </div>
            <a href="{{ route('Pengaju.create') }}" class="w-full md:w-auto flex items-center justify-center bg-teal-600 hover:bg-teal-700 text-white px-5 py-3 rounded-xl text-sm font-semibold transition-all shadow-lg shadow-teal-600/20">
                <i class="fas fa-plus mr-2"></i> Tambah Pengaju
            </a>
        </div>
                           ...
                           <td class="px-6 py-4 text-center">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('Pengaju.edit', $user->id) }}" class="p-2 text-slate-400 hover:text-blue-600"><i class="fas fa-edit"></i></a>
                                    @if($user->role != 1)
                                        <button @click="openDelete = true; deleteRoute = '{{ route('Pengaju.destroy', $user->id) }}'; PengajuNama = '{{ $user->nama }}'" class="p-2 text-slate-400 hover:text-red-600"><i class="fas fa-trash"></i></button>
                                    @endif
                                </div>
                            </td>
                        </tr>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <span class="px-2 py-0.5 text-[9px] font-bold uppercase rounded-full border {{ $user->role == 1 ? 'bg-purple-100 text-purple-700 border-purple-200' : 'bg-blue-100 text-blue-700 border-blue-200' }}">
                                    {{ $user->role == 1 ? 'Super Pengaju' : 'Pengaju' }}
                                </span>
                            </div>
                        </div>
                        <div class="flex gap-1">
                            <a href="{{ route('Pengaju.edit', $user->id) }}" class="p-2 text-blue-600 bg-blue-50 rounded-lg"><i class="fas fa-edit"></i></a>
                            @if($user->role != 1)
                                <button @click="openDelete = true; deleteRoute = '{{ route('Pengaju.destroy', $user->id) }}'; PengajuNama = '{{ $user->nama }}'" class="p-2 text-red-600 bg-red-50 rounded-lg"><i class="fas fa-trash"></i></button>
                            @endif
                        </div>
                    </div>
...
<div x-show="openDelete"
    ...
    <div class="flex min-h-full items-center justify-center p-4 text-center">
        <div x-show="openDelete"
            ...
            <h3 class="text-lg font-bold text-slate-900">Hapus Data Pengaju?</h3>

            {{-- Penambahan Nama Pengaju di sini --}}
            <div class="mt-2 p-3 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                <p class="text-xs text-slate-400 uppercase tracking-wider font-semibold">Pengaju yang dipilih:</p>
                <p class="text-sm font-bold text-red-600" x-text="PengajuNama"></p>
            </div>
